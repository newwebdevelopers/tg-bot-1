...
var notes_rus = {
  ...
  bot.onText(/\/start/, (msg, match) => {
    ...
    bot.sendMessage(chatId, 'pleas select you language', messageOptions.replay_markup);
  })
  bot.onText(/UZB/, (msg, match) => {
    ...
    if(check == 0) {
      ...
      bot.sendMessage(chatId, 'ok',messageOptions.replay_markup);
    }
  })
  bot.onText(/RUS/, (msg, match) => {
    ...
    if(check == 0){
      check = 1;
      lnge = notes_rus;
      bot.sendMessage(chatId, 'вы выбрали русский язык чем я могу вам помочь ?', messageOptions.remove_keyboard);
    }
  })
